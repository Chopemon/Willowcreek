...
class LocalLLMClient:
    def __init__(
        ...
        if gguf_path:
            ...
            llama_root = Path(llama_spec.origin or "").parent
            ggml_lib = llama_root / "lib" / "ggml.dll"
            if os.name == "nt" and not ggml_lib.exists():
                raise RuntimeError(
                    "llama-cpp-python is installed but its ggml.dll is missing. "
                    "Reinstall llama-cpp-python with the correct wheels for your system."
                )
