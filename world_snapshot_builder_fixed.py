...
Age: {malcolm.age} | Occupation: {malcolm.occupation}
...
- Mood: {mood_display.upper()}{bio_state}"""
    ...
    def _build_nearby_npcs(self, malcolm: 'NPC') -> str:
        ...
        nearby = [
            ...
            if npc.current_location == malcolm.current_location
            and npc.full_name != malcolm.full_name
        ]
        
        if not nearby:
            return f"## NEARBY NPCs\nNone - Malcolm is alone at {malcolm.current_location}"
        
        lines = [f"## NEARBY NPCs ({len(nearby)} present at {malcolm.current_location})"]
        ...
        for npc in nearby[:10]:
            ...
            lines.append(
                f"  • {npc.full_name} ({npc.age}, {npc.occupation})\n"
    ...
    def _build_goals(self) -> str:
        ...
        try:
            if hasattr(self.sim.goals, 'get_active_goals'):
...
def create_narrative_context(sim: 'WillowCreekSimulation', malcolm: 'NPC') -> str:
    """
    Convenience function to build full narrative context.
    """
