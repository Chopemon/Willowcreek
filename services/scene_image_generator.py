...
Supports both template-based and AI-generated prompts.
"""
...
class AIPromptGenerator:
    """
    Uses an LLM to generate image prompts from narrative text.
    Creates contextual, creative prompts that match the actual narrative.
    """
    ...
    def generate_prompt_from_narrative(self, narrative_text: str, context: SceneContext) -> Tuple[str, str]:
        """
        Generate ComfyUI prompts from narrative text using AI.

        Args:
            narrative_text: The actual narrative generated by the LLM
            context: Scene context for additional guidance

        Returns:
            Tuple of (positive_prompt, negative_prompt)
        """
        # Build prompt for the LLM
        system_prompt = """You are an expert at creating image generation prompts for Stable Diffusion/ComfyUI.
Your task is to analyze narrative text and create detailed, high-quality prompts that capture the scene visually.

Output format:
POSITIVE: [detailed positive prompt with quality tags, style, subjects, setting, mood]
NEGATIVE: [negative prompt with things to avoid]

Keep prompts concise but descriptive. Focus on visual elements, lighting, composition, and atmosphere."""

        user_prompt = f"""Create a ComfyUI image prompt from this narrative:
...
Generate a photorealistic, cinematic image prompt."""
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                return self._fallback_prompts(context)

            content = response.json()["choices"][0]["message"]["content"]

            # Parse the response
            positive, negative = self._parse_ai_response(content, context)

            print(f"\n[AIPromptGen] === AI-Generated Prompts ===")
            print(f"[AIPromptGen] Positive: {positive}")
            print(f"[AIPromptGen] Negative: {negative}")
            print(f"[AIPromptGen] ===========================\n")

            return positive, negative

        except Exception as e:
            print(f"[AIPromptGen] Error generating prompts: {e}")
            return self._fallback_prompts(context)

    def _parse_ai_response(self, response: str, context: SceneContext) -> Tuple[str, str]:
        """Parse AI response to extract positive and negative prompts."""
        positive = ""
        negative = ""

        # Look for POSITIVE: and NEGATIVE: markers
        ...
        if not positive:
            # Use the whole response as positive prompt, cleaned up
            positive = response.replace("POSITIVE:", "").replace("NEGATIVE:", "").strip()
            positive = positive.split('\n')[0]  # Take first line

        if not negative:
            # Use default negative prompt
            negative = "low quality, blurry, distorted, ugly, deformed, cartoon, anime, bad anatomy, text, watermark, logo"

        # Ensure quality tags are present
        if "masterpiece" not in positive.lower():
            positive = f"masterpiece, best quality, highly detailed, {positive}"

        return positive, negative

    def _fallback_prompts(self, context: SceneContext) -> Tuple[str, str]:
        """Fallback to template-based prompts if AI generation fails."""
        print("[AIPromptGen] Falling back to template-based prompts")
        template_gen = ImagePromptGenerator()
        return template_gen.generate_prompt(context)
